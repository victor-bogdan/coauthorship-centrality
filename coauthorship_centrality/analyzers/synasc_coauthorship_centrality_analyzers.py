...
class SYNASCCoauthorshipCentralityAnalyzer(CoauthorshipCentralityAnalyzer):
    ...
    def get_coauthorship_centrality(self, centrality_measure, analysis_type, year, year_period_type):
        ...
        if analysis_type == "author":
            for author_name, author_data in \
                    self._yearly_accumulative_analysis_data[year]['authors'].items():
                if centrality_measure.lower() == "closeness":
                    centrality_dict[author_name] = author_data['author_closeness_centrality']
                elif centrality_measure.lower() == "betweenness":
                    centrality_dict[author_name] = author_data['author_betweenness_centrality']
                elif centrality_measure.lower() == "degree":
                    centrality_dict[author_name] = author_data['author_degree_centrality']
                elif centrality_measure.lower() == "pagerank":
                    centrality_dict[author_name] = author_data['author_pagerank_centrality']
        elif analysis_type == "collaborator_group":
            for collaborator_group_id, collaborator_group_data in \
                    self._yearly_accumulative_analysis_data[year]['collaborator_groups'].items():
                if centrality_measure.lower() == "closeness":
                    centrality_dict[collaborator_group_id] = \
                        collaborator_group_data['collaborator_groups_closeness_centrality']
                elif centrality_measure.lower() == "betweenness":
                    centrality_dict[collaborator_group_id] = \
                        collaborator_group_data['collaborator_groups_betweenness_centrality']
                elif centrality_measure.lower() == "degree":
                    centrality_dict[collaborator_group_id] = \
                        collaborator_group_data['collaborator_groups_degree_centrality']
                elif centrality_measure.lower() == "pagerank":
                    centrality_dict[collaborator_group_id] = \
                        collaborator_group_data['collaborator_groups_pagerank_centrality']
    ...
    def __init_data(self, year_period_type):
        ...
        for file_path in file_paths:
            with open(file_path, "r", encoding="utf-8-sig") as file:
                ...
                self._yearly_accumulative_analysis_data[str(data[year_key])] = data
